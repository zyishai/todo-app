...
test('the default category should be "Default"', () => {
  const task = new Task();

  expect(task.category).to.match(/Default/i);
});

test('calling setCategory should update the category field', () => {
  const task = new Task();
  task.setCategory('special');

  expect(task.category).to.equal('special');
});

test('calling toJSON should return JSON represantation of the object', () => {
